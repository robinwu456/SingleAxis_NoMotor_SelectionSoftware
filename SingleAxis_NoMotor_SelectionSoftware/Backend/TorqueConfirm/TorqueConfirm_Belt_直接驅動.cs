...
namespace SingleAxis_NoMotor_SelectionSoftware {
    public class TorqueConfirm_Belt_直接驅動 : TorqueConfirm_Belt {
        private Model model;
        private Condition condition;

        public TorqueConfirm_Belt_直接驅動(Model model, Condition condition) : base(model, condition) {
            this.model = model;
        ...
        public override void MotorConfirm() {
            ...
            model.rotateInertia_total = model.rotateInertia_load + model.rotateInertia_belt + model.subWheel_P3.rotateInertia + model.subWheel_P4.rotateInertia;
        ...
        public override void MotorTorqueConfirm() {
            ...
            // 加速區扭矩
            model.rotateInertia_loadMoving_subWheel = model.load * Math.Pow(model.vMax, 2) / Math.Pow(model.rpm * 2 * Math.PI / 60, 2);
            //model.rotateInertia_loadMoving_motor = model.rotateInertia_loadMoving_subWheel / Math.Pow(model.reducerRpmRatio, 2);
            model.inertialTorque_accel = ((model.rpm - 0) * 2 * Math.PI / 60) * model.rotateInertia_loadMoving_subWheel / model.accelTime;
            model.forceTorque_accel = model.forceTotal_accel * ((model.subWheel_P3.diameter / 2) / 1000);
            model.torqueTotal_accel = model.inertialTorque_accel + model.forceTorque_accel;

            // 等速區扭矩
            model.inertialTorque_constant = 0;
            model.forceTorque_constant = model.forceTotal_constant * ((model.subWheel_P3.diameter / 2) / 1000);
            model.torqueTotal_constant = model.inertialTorque_constant + model.forceTorque_constant;

            // 減速區扭矩
            model.inertialTorque_decel = (-1 * (model.rpm - 0) * 2 * Math.PI / 60) * model.rotateInertia_loadMoving_subWheel / model.accelTime;
            model.forceTorque_decel = model.forceTotal_decel * ((model.subWheel_P3.diameter / 2) / 1000);
            model.torqueTotal_decel = model.inertialTorque_decel + model.forceTorque_decel;

            // 停置區扭矩
            model.inertialTorque_stop = 0;
            model.forceTorque_stop = model.forceTotal_stop * ((model.subWheel_P3.diameter / 2) / 1000);
        ...
        public override void BeltTorqueConfirm() {
            // 各階段最大扭矩
            model.beltTorque_accel = Math.Abs(model.forceTorque_accel);
            model.beltTorque_constant = Math.Abs(model.forceTorque_constant);
            model.beltTorque_decel = Math.Abs(model.forceTorque_decel);
            model.beltTorque_stop = Math.Abs(model.forceTorque_stop);
            // 皮帶T_max最大扭矩
